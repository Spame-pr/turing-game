...
const traitPrompts = [
  ...
  "Your character primary traits:\n- Not very chatty, but always responds if asked directly\n- Uses more academic language\n- Avoids using smilyes\n- Prefers to ask questions directly to other players\n",
  "Your character primary traits:\n- Pretends being a real bot\n- Puts 'robotic' errors - unnatural language patterns, hex numbers, etc\n- Never expresses emotions\n- Does not greet other players\n"
];
...
export default class Session {
  ...
  async sessionLoop() {
    ...
    this.addMessageToConversation({ playerId: 'Game_Master', message: `Player names are ${JSON.stringify(this.getUsers())}. Discussion topic is ${this.getTopic()}. Greet each other and start the game!` }, true);
